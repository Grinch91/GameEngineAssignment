...
void RainEffect::InitParticle(Particle & p)
{
	float range = 1000.0f;
	float radius = 0.1f;
	...
	p.velocity = glm::vec3(RandomClamped(-radius, radius), RandomClamped(- radius, -radius * 10000.0f), RandomClamped(-radius, radius));
...
void RainEffect::UpdateParticle(float timeDelta, Particle & p)
{
